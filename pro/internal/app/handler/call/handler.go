...
func New(registry *mmodem.Registry, calls *procall.Service, media *procall.Media) *Handler {
	return &Handler{registry: registry, calls: calls, media: media}
}
...
func (h *Handler) Update(c *echo.Context) error {
	...
	call, err := h.calls.Update(c.Request().Context(), modem, callIDParam(c), procall.UpdateRequest{
		State:  req.State,
		Reason: req.Reason,
		Hold:   req.Hold,
	})
...
func (h *Handler) serveWebRTCSession(ctx context.Context, conn *websocket.Conn, modem *mmodem.Modem, callID string) error {
	...
	for {
		...
		case "offer":
			...
			answer, err := session.Answer(ctx, procall.WebRTCSessionDescription{
		...
		case "candidate":
			...
			if err := session.AddICECandidate(procall.WebRTCICECandidate{
...
func writeWebRTCIceCandidates(ctx context.Context, conn *websocket.Conn, writeMu *sync.Mutex, candidates <-chan procall.WebRTCICECandidate) {
	for {
		select {
		case <-ctx.Done():
			return
...
func buildWebRTCICEServersResponse(servers []procall.WebRTCICEServer) WebRTCICEServersResponse {
